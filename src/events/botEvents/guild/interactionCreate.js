...
module.exports.run = async (client, interaction) => {
    if (interaction.type === InteractionType.ApplicationCommand) {
        ...
        if (command.settings.player && !player) {
            warning.setDescription(`\`❌\` | No hay jugador existe para este servidor.`);
        ...
        if (command.settings.current && !player.currentTrack) {
            warning.setDescription(`\`❌\` | No hay ninguna canción en reproducción en este momento.`);
        ...
        if (command.settings.inVc) {
            if (!channel) {
                warning.setDescription(`\`❌\` | Debes estar en un canal de voz para utilizar este comando.`);
            ...
            if (
                ...
                warning.setDescription(
                    `\`❌\` | No tengo permiso \`${command.permissions.channel.join(
                        ", ",
                    )}\` para ejecutar este comando en este canal.`,
                );
        ...
        if (command.settings.sameVc) {
            if (!channel || interaction.member.voice.channelId !== interaction.guild.members.me.voice.channelId) {
                warning.setDescription(`\`❌\` | Debes estar en el mismo canal de voz que el mío para utilizar este comando.`);
        ...
        if (command.settings.owner && interaction.user.id !== client.owner) {
            warning.setDescription(`\`❌\` | Sólo mi dueño puede usar este comando.`);
        ...
        if (command.settings.premium) {
            if (client.config.disablePremium === false && user && !user.isPremium) {
                warning.setDescription(`\`❌\` | No eres usuario premium!`);
        ...
        try {
            ...
            warning.setDescription(`\`❌\` | Algo salió mal.`);
